...
- Room 2.6.1 via KSP: `AppDatabase` version 9 (+ `MIGRATION_8_9` for the RSS FTS index), `exportSchema = false`, `fallbackToDestructiveMigration()`. Bump `version` on schema changes; data is wiped unless you add a manual migration to `addMigrations(...)` (the FTS migration creates the FTS4 table and its sync triggers — keep them byte-identical to Room's generated statements).
